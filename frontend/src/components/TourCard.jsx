...
export function TourProvider({ children }) {
  ...
  const start = useCallback(() => { setIdx(0); setActive(true); setAuto(true); }, [setIdx, setActive, setAuto]);
  const stop = useCallback(() => { setActive(false); setAuto(false); }, [setActive, setAuto]);
  return (
...
// Navigates to the step route and runs its scripted action exactly once per step entry.
function useStepRunner({ active, idx, setBusy }) {
...
function useAutoAdvance({ active, auto, busy, idx, setIdx, setAuto }) {
  useEffect(() => {
...
export default function TourCard() {
  ...
  useStepRunner({ active, idx, setBusy });
  useAutoAdvance({ active, auto, busy, idx, setIdx, setAuto });
...
function TourHeader({ L, idx, onExit }) {
  return (
    <div className="flex items-center gap-3 text-[10.5px] uppercase tracking-[0.16em]" style={{ color: L.accent }}>
      ...
      <button onClick={onExit} className="rounded-full p-1 hover:bg-white/10" data-testid="tour-exit-btn" aria-label="Exit story"><X size={15} style={{ color: "#8FA6A0" }} /></button>
    </div>
  );
}

function TourControls({ L, idx, auto, busy, setAuto, setIdx, stop }) {
  const last = idx === LAST;
  return (
    <div className="flex items-center gap-2 mt-4">
      <button onClick={() => setAuto(!auto)} data-testid="tour-autoplay-btn" className="chip" style={{ color: L.accent, background: L.soft, border: `1px solid ${L.accent}55` }}>
